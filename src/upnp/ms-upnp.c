			...
			gpointer user_data)
{
  ...
  media = build_media_from_didl (didl_node, bs->keys);
		 ...
		 gpointer user_data)
{
  ...
  bs = (MsMediaSourceBrowseSpec *) user_data;
  ...
  /* Use bs->count to emit "remaining" information */
  if (bs->count > returned) {
    bs->count = returned;
  }
