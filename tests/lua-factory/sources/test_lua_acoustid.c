...
test_query (void)
{
  ...
  gchar *basic_query = "duration=445&fingerprint=";
  query_size = strlen(&basic_query) + strlen(&fingerprint) + 1;
  g_strlcat(query, basic_query, query_size);
  g_strlcat(query, fingerprint, query_size);
