...
test_query_by_fingerprint (void)
{
  ...
  gsize size;
  GrlSource *source;
  gchar *duration_tag = "duration=";
  gchar *fingerprint_tag = "fingerprint=";

  file = g_file_new_for_uri (FINGERPRINT_LUDOVICO_EI);
  g_file_load_contents (file, NULL, &fingerprint, &size, NULL, &error);

  gchar *query = g_strconcat(duration_tag, "445", "&", fingerprint_tag, fingerprint, NULL);
...
main (gint argc, gchar **argv)
{
  ...
  g_test_add_func ("/lua_factory/sources/acoustid_resolve_by_fingerprint",
                   test_resolve_fingerprint);
  g_test_add_func ("/lua_factory/sources/acoustid_query_by_fingerprint",
