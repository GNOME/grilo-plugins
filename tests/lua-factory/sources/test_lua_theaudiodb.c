...
test_resolve_artist_art (void)
{
  ...
  struct {
    ...
    gchar *first_thumbnail;
    gchar *second_thumbnail;
    gchar *third_thumbnail;
    gchar *fourth_thumbnail;
    gchar *fifth_thumbnail;
  } audios[] = {
   { "coldplay",
     5,
  ...
  for (i = 0; i < G_N_ELEMENTS (audios); i++) {
    ...
    g_assert_cmpstr (grl_media_get_thumbnail_nth (GRL_MEDIA (media), 0), ==, audios[i].first_thumbnail);
    g_assert_cmpstr (grl_media_get_thumbnail_nth (GRL_MEDIA (media), 1), ==, audios[i].second_thumbnail);
    g_assert_cmpstr (grl_media_get_thumbnail_nth (GRL_MEDIA (media), 2), ==, audios[i].third_thumbnail);
    g_assert_cmpstr (grl_media_get_thumbnail_nth (GRL_MEDIA (media), 3), ==, audios[i].fourth_thumbnail);
    g_assert_cmpstr (grl_media_get_thumbnail_nth (GRL_MEDIA (media), 4), ==, audios[i].fifth_thumbnail);
